            ...
            <Document>
                <Page size="A4" style={styles.body}>
                    <View>
                        ...
                        <Text style={styles.datosFactura}>Estado de servicio:{servicios[0].procesoServicio}</Text>
                        ...
                        {servicios[0].tipoServicio === "VENTA"
                            ...
                            :
                            <Table
                                data={servicios[0].procedimientos}
                            >
                                <TableHeader>
                                    <TableCell>
                                        Nombre
                                ...
                                {
                                    servicios[0].procedimientos.map((element, index) => {
                                        return (
                                            <TableBody key={index}>
                        ...
                        {
                            servicios[0].tipoServicio === "ARREGLO"
                                ? <Text style={{ textAlign: 'right', fontSize: 15, marginRight: 140 }}>{`TOTAL: ${pesoColombiano.format(servicios[0].precioTotal)}`}</Text>
                                : <Text style={{ textAlign: 'right', fontSize: 15, marginRight: 80 }}>{`TOTAL: ${pesoColombiano.format(servicios[0].precioTotal)}`}</Text>
                        }
                        {
                            servicios[0].tipoServicio === "ARREGLO"
                ...
                servicios.length > 0
                    ? <Grid container>
                        <Grid item md={12} xs={12}>
                            {!pdgGenerate && <Button style={{ marginBottom: 20 }} onClick={() => setPdgGenerate(true)} fullWidth color="secondary" variant="contained">Generar PDF</Button>}
                            {
                                pdgGenerate
                    ...
                    : <Grid container >
                        <Grid item md={12} xs={12}>
                            <form onSubmit={handleSubmit(onSubmit)} style={{ textAlign: 'center' }}>
                                ...
                                />
                                <br />
                                <Button style={{ marginTop: 5 }} variant="contained" type="submit" color="primary" endIcon={<SearchIcon />} size="large">
                                    consultar
                                </Button>
                            </form>
